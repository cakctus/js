...
// 'xx xax xaax xaaax'.replace(/xa{1,2}x/g, '!');

// Начало '^' и конец '$' строки
// 'aaa aaa aaa'.replace(/^aaa/g, '!');
// 'aaa aaa aaa'.replace(/aaa$/g, '!');
// 'aaa aaa aaa'.replace(/^aaa$/g, '!');

// 'Или' через вертикальную черту |
// 'bbbb'.replace(/a|b+|c/g, '!');
// 'axx bxx bbxx exx'.replace(/(a|b+)xx/g, '!');

// console.log(
//   "carmen norman xsx x&x x-x xaaax \n new string \n new string".match(/\n.+/g)
// )

// Повторение
// console.log("carmen xsx x&x x-x xaaax".match(/x.x/g, "r"))
// console.log("carmen x xssssx x&x x-sx xassaax".match(/x.+x/g))
// console.log("carmen x xabababx xababx x&x x-sx xassaax".match(/x(ab)*x/g))
// console.log("carmen x xssssx x&x x-sx xassaax".match(/x.*x/g))
